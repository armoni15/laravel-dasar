...
class Encript
{
  ...
  public static function enkripsi($data)
  {
    ...
    foreach ($data_arr as $isi) {
      if ($i % 2 == 0) {
        $cipherText .= Encript::geserteks($isi, -$i);
        $i++;
      } else {
        $cipherText .= Encript::geserteks($isi, $i);
        $i++;
  ...
  public static function geserteks($string, $key)
  {
    return implode('', array_map(function ($char) use ($key) {
      return Encript::geserkarakter($char, $key);
    }, str_split($string)));
  }
  ...
  public static function geserkarakter($char, $shift)
  {
    ...
    if ($ascii >= 65 && $ascii <= 90) {
      return chr(Encript::geserhurufbesar($shifted));
    }

    if ($ascii >= 97 && $ascii <= 122) {
      return chr(Encript::geserhurufkecil($shifted));
    }

    if ($ascii >= 33 && $ascii <= 58) {
      return chr(Encript::geserangka($shifted));
    }
